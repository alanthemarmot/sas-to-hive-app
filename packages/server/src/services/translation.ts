...
## Line Mapping
After the SQL output, provide a line mapping as a \`\`\`json block:
...
Include one mapping entry per logical SAS construct. Aim for 3-8 mappings per translation. Each id should be a short kebab-case identifier (e.g. "proc-sort", "select-clause"). The explanation should be written for a non-technical SAS user.`;
...
export function parseTranslationResponse(response: string): {
  ...
  return { hiveSQL: sql, explanation, mappings };
}
